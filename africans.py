...
# /w/api.php?action=query&prop=info&format=json&titles=Wikipedia%7CDave%20Coulier%7CAntarctica: if ["query"]["pages"] has a negative int like -1, -2, etc. as a key, and if a key within that dict has the value "missing" (value: ""), then the page is missing from enwiki

def leftout(origlist, formattedlist):
    unsung = []
    for x, elem in enumerate(formattedlist):
        payload = dict(title=elem)
...
# (to find out who actually did have an entry, do a simple set operation for difference between set(pagelist) and set(starterlist)
